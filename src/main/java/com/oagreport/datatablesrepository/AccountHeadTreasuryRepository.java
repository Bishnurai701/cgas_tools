...
public interface AccountHeadTreasuryRepository extends DataTablesRepository<TmsAccountHead, Long> {
	...
	long getMax();
	  ...
	  @Query(value =
	  "select a from TmsAccountHead a join MiscAccountType t on a.accountTypeId=t.id order by t.sortOrder asc "
	...
	/*
	 * @Query(value =
	 * "select a from TmsAccountHead a join MiscAccountType t on a.accountTypeId=t.id where a.accountTypeId=:groupId order by t.sortOrder asc "
	 * ) Iterable<TmsAccountHead> getAccountHeadByGroup(@Param("groupId") Integer
	 * groupId);
	 */
